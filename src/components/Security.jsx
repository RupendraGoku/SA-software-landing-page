        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          <div className="lg:w-1/2">
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              ...
              <p className="text-zinc-400 text-lg leading-relaxed mb-8">{content.description}</p>

              <ul className="space-y-4">
                {content.controls.map((control) => (
                  <li key={control} className="flex items-center gap-3 text-zinc-300">
              ...
              {content.certifications.map((certification) => {
                ...
                return (
                  <div
                    ...
                    className="group relative p-8 bg-zinc-900/50 border border-white/5 rounded-2xl backdrop-blur-md overflow-hidden hover:border-emerald-500/30 transition-colors"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

                    <Icon className="w-10 h-10 text-emerald-500 mb-4" />
                    <h4 className="text-xl font-bold text-white">{certification.name}</h4>
                    <p className="text-xs text-zinc-500 mt-2">{content.certificationLabel}</p>
                  </div>
                );
              })}
            </motion.div>
